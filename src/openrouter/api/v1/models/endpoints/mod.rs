...
pub(super) fn config(cfg: &mut web::ServiceConfig) {
    cfg.service(endpoints);
}

#[get("/api/v1/models/{author}/{slug}/endpoints")]
async fn endpoints(params: web::Path<PathParams>) -> impl Responder {
    let client: reqwest::Client = reqwest::Client::new();

    let (status, body): (
        StatusCode,
        response::Response<response::ListEndpointsModel>,
    ) = async {
        let result: reqwest::Response = client
            .get(format!(
                "{BASE_URL}models/{}/{}/endpoints",
                params.author, params.slug
            ))
            .send()
            .await?;
        let status: StatusCode = result.status().as_u16().try_into()?;
        let body: response::Response<response::ListEndpointsModel> = {
            #[cfg(debug_assertions)]
            {
                serde_path_to_error::deserialize::<
                    _,
                    response::ListEndpointsModel,

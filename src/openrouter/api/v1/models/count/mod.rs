...
async fn count() -> impl Responder {
    ...
    let (status, body): (
        StatusCode,
        response::Response<response::TotalCountAvailableModels>,
    ) = async {
        let result: reqwest::Response =
            client.get(format!("{BASE_URL}models/count")).send().await?;
        let status: StatusCode = result.status().as_u16().try_into()?;
        let body: response::Response<response::TotalCountAvailableModels> = {
            #[cfg(debug_assertions)]
            {
                serde_path_to_error::deserialize::<
                    _,
                    response::TotalCountAvailableModels,

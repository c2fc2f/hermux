...
async fn models() -> impl Responder {
    ...
    let (status, body): (
        StatusCode,
        response::Response<response::ListModelsFilteredUserPreferences>,
    ) = async {
        let result: reqwest::Response =
            client.get(format!("{BASE_URL}models/user")).send().await?;
        let status: StatusCode = result.status().as_u16().try_into()?;
        let body: response::Response<
            response::ListModelsFilteredUserPreferences,
        > = {

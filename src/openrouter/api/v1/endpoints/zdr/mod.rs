...
async fn endpoints() -> impl Responder {
    let client: reqwest::Client = reqwest::Client::new();

    let (status, body): (
        StatusCode,
        response::Response<response::ListEndpointsZDR>,
    ) = async {
        let result: reqwest::Response = client
            .get(format!("{BASE_URL}endpoints/zdr"))
            .send()
            .await?;
        let status: StatusCode = result.status().as_u16().try_into()?;
        let body: response::Response<response::ListEndpointsZDR> = {
            #[cfg(debug_assertions)]
            {
                serde_path_to_error::deserialize::<
                    _,
                    response::ListEndpointsZDR,
